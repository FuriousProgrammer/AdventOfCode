-- MODIFIED Adam Albee 2019
-- Added __mod metamethod within existing format.
-- Patched to work with Lua 5.3 (tostring modification).
...
function BigNum.mt.mod( num1 , num2 )
   local _, res = BigNum.mt.div( num1, num2 ) ;
   return res ;
end
...
function BigNum.mt.tostring( bnum )
   ...
   elseif bnum.len > 0 then
      for i = bnum.len - 2 , 0 , -1  do
         ...
         temp = temp .. math.floor(bnum[i]) ;
      end
      temp = math.floor(bnum[bnum.len - 1]) .. temp ;
      if bnum.signal == '-' then
